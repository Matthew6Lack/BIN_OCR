...
enum WorkerResponse {
    ...
    LoadDone(Vec<Vec<u32>>, String), // Modifié ici : Vec<Vec<u32>> à la place de Vec<Vec<u8>>
    SolveDone(i32, i32),
    Error(String),
}
...
impl BinaryViewer {
        ...
        match message {
            ...
            Message::Load => {
                ...
                self.send_to_worker(WorkerRequest::Load {
                    image_path: path,
                    liste_images: self.liste_images.clone(),
                })
            }
            Message::Save => {
            ...
            Message::WorkerReply(resp) => {
                match resp {
                    ...
                    WorkerResponse::LoadDone(_pixels, path) => {
                        self.status_text = format!("Chargé avec succès : {path}");
                        self.loaded_image_path = Some(path.clone());

                        // AJOUT : Si l'image n'est pas encore dans notre carrousel, on l'ajoute !
                        if !self.liste_images.contains(&path) {
                            self.liste_images.push(path);
                        }
                        // Optionnel : On place directement le curseur de l'application sur cette nouvelle image
                        self.current_index = self.liste_images.len() - 1;
                    }
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        .spacing(50) //
        .align_y(Vertical::Center);
        let input_field = text_input("Entrez votre texte ici…", &self.user_text)
    ...
    while let Ok(req) = rx.recv() {
        let resp = match req {
            ...
            WorkerRequest::Load {
                ...
                match load_img(&mut liste_images) {
                    Some((matrix, name)) => {
                        let final_path = format!("src/img/{}", name);
                        WorkerResponse::LoadDone(matrix, final_path)
                    }
                    None => {
                        WorkerResponse::Error("Sélection d'image annulée ou invalide".to_string())
                    }
                }
            }
