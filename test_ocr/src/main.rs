...
fn main() {
    ...
    match run_ocr(&"src/images/BINimg2.png") {
        Some((nb_signed, nbin)) => {
            println!("Succès !");
            println!("  Valeur binaire brute (non signée) : {}", nbin);
            println!("  Valeur finale décimale (signée)   : {}", nb_signed);
        }
        None => println!("Échec lors de l'exécution de run_ocr."),
    }
}
