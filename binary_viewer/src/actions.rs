use crate::{
    ...
    midsized_letter::*, neural_network::*, read_csv::*, resizing::*, solution::*,
};

pub fn run_ocr(path: &str) -> Option<(i32, i32)> {
    if let Err(e) = clear_digits_folder() {
        eprintln!(
            "Attention, impossible de vider le dossier 'digits' : {:?}",
            e
        );
        return None;
    }
    ...
    }else{println!("flatten empty");}

    Some((nb_signed, nbin))
}
pub fn extract_position(path: &str) -> Option<Vec<(usize, usize, usize, usize)>> {
    let (pgm_path, w, h) = convert_to_pgm(&path).ok()?;
    let bin = binarize_pgm(&pgm_path, w, h).ok()?;
    if bin.is_empty() {
        println!("Erreur : Le vecteur binaire est vide!");
